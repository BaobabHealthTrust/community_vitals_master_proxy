...
rs = $con.query("SELECT cvr_mtema_live.person_name.person_id as id, identifier as national_id, city_village as creator_site_id, given_name, middle_name, family_name, gender, birthdate, birthdate_estimated, city_village as village, cvr_mtema_live.person_name.date_created as created_at FROM cvr_mtema_live.person_name LEFT OUTER JOIN cvr_mtema_live.person ON cvr_mtema_live.person.person_id = cvr_mtema_live.person_name.person_id LEFT OUTER JOIN cvr_mtema_live.person_address ON cvr_mtema_live.person_address.person_id = cvr_mtema_live.person.person_id LEFT OUTER JOIN cvr_mtema_live.patient ON cvr_mtema_live.patient.patient_id = cvr_mtema_live.person.person_id LEFT OUTER JOIN cvr_mtema_live.patient_identifier ON cvr_mtema_live.patient_identifier.patient_id = cvr_mtema_live.patient.patient_id WHERE cvr_mtema_live.patient_identifier.identifier_type = 3 AND cvr_mtema_live.patient_identifier.voided = 0;") 
