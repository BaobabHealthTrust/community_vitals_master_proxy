module CoreOpenmrs
  ...
  def before_save
    super
    self.changed_by = 1 if self.attributes.has_key?("changed_by") and (self.changed_by.blank? || self.changed_by == 0)
  ...
  def before_create
    ...
    self.location_id = 1 if self.attributes.has_key?("location_id") and (self.location_id.blank? || self.location_id == 0)

    self.creator = 1 if self.attributes.has_key?("creator") and
        (self.creator.blank? || self.creator == 0)  # temporary
