P.1. Site Setup [program => COMMUNITY VITALS REGISTRATION $$ action => javascript:updateSelectedUser()]
...
Q.1.5. Password [pos => 5 $$ field_type => password $$ condition => (window.location.href.match(/\?/) ? (window.location.href.match(/password/i) ? true : false) : true) $$ tt_onLoad => current_case_upper = false; $$ tt_onUnLoad => __$("1.6").setAttribute("validationRule", __$("inputField").value.trim())]
...
Q.1.7. User Roles [pos => 7 $$ tt_showToggleKeyboard => false $$ showKeys => false $$ dynamicLoader => listFilteredUserRoles(__$('inputField').value.trim()) $$ condition => (window.location.href.match(/\?/) ? (window.location.href.match(/role/i) ? true : false) : true) $$ tt_onLoad => listFilteredUserRoles('')]
